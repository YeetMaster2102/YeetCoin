...
#if defined(NDEBUG)
# error "Yeetcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x8aa4159c29902ca6b18a4231bb54e7504e71da9e0e88290d0661ccfe833b9af3");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Yeetcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Yeetcoin Signed Message:\n";
...
{
    // Yeetcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Yeetcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3 * 24 * 60 * 60; // Yeetcoin: 3.5 days
static const int64 nTargetSpacing = 2 * 60; // Yeetcoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Yeetcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Yeetcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x4ea909e234c8ea3ef1baa68b84f1883cff95a1c07d081c111b2a120af58b9ddd");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "April 1, 2021 Covid 19 Vaccine proves 100% effective in kids";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 30 * COIN;
        txNew.vout[0].scriptPubKey = CScript() <<ParseHex("045ddac91ffbfa6a291c025420fdd9b88d9c6297a446b405c5c1eba5777e6b9518d1e2e74dd3951aedd384fa6bd2e5daeeb59167b1e05991b6f86fa2d237c86f11")<<OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1617289824;
;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084955319;

        if (fTestNet)
        {
            block.nTime    = 1617289796;
;
            block.nNonce   = 386426707;
        }
...
if (true && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x217f250300ff8e4141f07cb2c35c43d79246e7002495c4b41da9bcfdb5d5f2c6"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfl, 0xc3, 0xb9, 0xde }; // Yeetcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("YeetcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("YeetcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static YeetcoinMiner(CWallet *pwallet)
{
    printf("YeetcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("yeetcoin-miner");
    ...
    try { loop {
        ...
        printf("Running YeetcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("YeetcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&YeetcoinMiner, pwallet));
}
